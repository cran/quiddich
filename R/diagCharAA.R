#' Identification of diagnostic molecular characters in amino acid alignments
...
#'
#' @param AAbin An object (the amino acid alignment) of class 'AAbin'.
...
#'
#' @return \code{diagCharAA} returns a list, where each entry belongs to one
...
#'
#' \code{diagCharAA} returns for each taxon of interest the following elements:
...
#' \item{compared taxa}{Only relevant for type 4 characters. It contains the
...
#' @importFrom ape AAsubst

#' @export
diagCharAA <- function(AAbin, taxVector=dimnames(AAbin)[[1]], taxOfInt="all", types=c("type1","type2","type3"), gapValid=TRUE)
{
  # transform dataset (alignment) into matrix
  AAbin <- as.matrix(AAbin)
  # extract polymorphic sites in the alignment (positions of interest)
  inform <- ape::AAsubst(AAbin)
  ...
  for(i in taxSeqsPos)
  {
    ...
    for(j in 1:length(inform))
    {
      ...
      states_i <- unique(as.vector(as.character(AAbin[taxSeqs[[i]],inform[j]])))
      states_rest <- unique(as.vector(as.character(AAbin[-taxSeqs[[i]],inform[j]])))
      ...
      if(!("X" %in% states_rest))
      {
        for(k in 1:length(states_i))
        {
          ...
          if(a != "X" && !(a %in% states_rest) && isTRUE(gapValid | a != "-"))
          {
            newpos[2] <- "type3"
            newpos[3] <- paste(newpos[3], a, sep="")
            # for j to be of type1/2, states(i,j) and states(rest,j) cannot share any states
            # and states(i,j) cannot contain X
            if(!(any(states_i %in% states_rest)) && !("X" %in% states_i))
            {
              newpos[2] <- "type2"
      ...
      {
        # for j to be of type4, it must be fixed in taxon i for any state except "X"
        # (and "-" if gaps are not valid states)
        if(length(states_i)==1 && states_i[1]!="X" && isTRUE(gapValid | states_i[1]!="-"))
        {
          ...
          for(l in 1:length(taxSeqs))
          {
            if(l != i)
            {
              ...
              states_l <- unique(as.vector(as.character(AAbin[taxSeqs[[l]],inform[j]])))
              # for j to be of type4, taxon l must be fixed for one
              # state that is different from "X" and different from the state of taxon i
              # (and different from "-" if gaps are not valid states)
              if(length(states_l)==1 && states_l[1]!="X" && states_i[1]!=states_l[1] && isTRUE(gapValid | states_l[1]!="-"))
              {
                newpos[1] <- inform[j]
                newpos[2] <- "type4"
                newpos[3] <- states_i[1]
